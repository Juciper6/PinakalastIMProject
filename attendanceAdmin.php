...
<head>
    ...
    <title>Admin - PLATFORM USER</title>
...
<body class="bg-gray-300 bg-no-repeat" style="background-image: url('pic/social-media.webp'); background-position: center; background-size: cover; font-family: 'Roboto', Arial, sans-serif; height: 100vh;">
            ...
            <div class="flex justify-between items-center py-3 relative">
                <div class="flex items-center space-x-3">
                    ...
                    <div class="text-lg font-bold text-gray-800 hidden sm:block">
                        <span class="text-sm">ADMIN  [ J & J IM SOCIAL PLATFORM ] - </span>
                    </div>
                </div>
        ...
        <div class="mt-4 grid grid-cols-2 gap-4">
            ...
            try {
                ...
                if (empty($sections)) {
                    ...
                    echo "<h2 class='text-xl font-semibold text-gray-700'>No Users Found</h2>";
                    echo "<p class='text-gray-600 mt-2'>There are currently no users in the system.</p>";
                    echo "</div>";
                } else {
                    ...
                    foreach ($sections as $section) {
                        ...
                        try {
                            ...
                            echo "<h2 class='text-center text-xl md:text-2xl font-semibold text-gray-700 mb-4'>Platform " . $currentSectionDisplay . " (<span class='font-mono'>" . count($results) . "</span>Users)</h2>";
                            echo "<a href='javascript:void(0);' onclick='confirmClearSection(\"{$currentSectionName}\")' class='text-black-600 hover:text-black-800 transition duration-150 ease-in-out' title='Clear Attendance for this platform'><div class='flex justify-center'><i class='fas fa-archive fa-fw'></i></div></a>";
                            echo "<br>";
                            ...
                            echo "<th class='border px-3 py-3 text-center'>ID</th>";
                            echo "<th class='border px-3 py-3 text-left'>Username</th>";
                            echo "<th class='border px-3 py-3 text-left'>Nickname</th>";
                            echo "<th class='border px-3 py-3 text-center'>Time In</th>";
                                ...
                                foreach ($results as $result) {
                                    ...
                                    if (!empty($result['date_attended'])) {
                                        try {
                                            ...
                                            $timeDisplay = $dateTime->format('g:i FACEBOOK');
                                            if ($timeDisplay <= $time) {
                            ...
                            error_log("Error fetching users details for this platform {$currentSectionDisplay} on admin page: " . $e->getMessage());
                            echo "<tr><td colspan='5' class='border px-4 py-3 text-center text-red-500 font-semibold'>Error loading user's data for this platform.</td></tr>";
                        }
                    }
                }
            } catch (PDOException $e) {
    ...
    <script>
        ...
        function confirmClearSection(section) {
            Swal.fire({
                title: "Clear users for this platform " + section + "?",
                text: "You won't be able to revert this!",
                icon: 'warning',
                showCancelButton: true,
                confirmButtonColor: '#d33',
