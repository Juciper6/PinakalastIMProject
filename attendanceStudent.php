...
<body class="bg-gray-300 bg-no-repeat" style="background-image: url('pic/social-media.webp'); background-position: center; background-size: cover; font-family: 'Roboto', Arial, sans-serif; height: 100vh;">
        ...
        <div class="container mx-auto px-4">
            <div class="flex justify-between items-center py-3 relative">
                ...
                <div class="hidden md:flex space-x-4 nav-links" id="navLinks">
                    <a href="#FACEBOOK" class="text-gray-600 hover:text-blue-600 px-3 py-2 rounded-md text-sm font-medium">FACEBOOK</a>
                    <a href="#INSTAGRAM" class="text-gray-600 hover:text-blue-600 px-3 py-2 rounded-md text-sm font-medium">INSTAGRAM</a>
                    <a href="#TWITTER" class="text-gray-600 hover:text-blue-600 px-3 py-2 rounded-md text-sm font-medium">TWITTER</a>
                    <a href="#TELEGRAM" class="text-gray-600 hover:text-blue-600 px-3 py-2 rounded-md text-sm font-medium">TELEGRAM</a>
                    <a href="form.php" class="text-gray-600 hover:text-blue-600 px-3 py-2 rounded-md text-sm font-medium">Edit Name</a>
    ...
    <div class="container mx-auto p-4 md:p-6 lg:p-8 pt-20">
        ...
        <h1 class="text-xl text-center text-gray-800 m-2">User's Today</h1>
        ...
        ?>
                    <div id="<?php echo $currentSection; ?>"><br>
                        <div class="my-6 bg-white p-4 rounded shadow-md">
                            <h2 class='text-center text-xl font-semibold text-gray-700 my-2'>Platform <?php echo htmlentities($currentSection); ?></h2>
                            <div class="overflow-x-auto">
                                <table class='table-auto w-full border-collapse border border-gray-300'>
                                    <thead>
                                        <tr class='bg-gray-100 text-gray-600 uppercase text-sm leading-normal'>
                                            <th class='border px-3 py-2 text-center'>No.</th>
                                            <th class='border px-3 py-2 text-left'>Users</th>
                                            <th class='border px-3 py-2 text-left'>Nickname</th>
                                            <th class='border px-3 py-2 text-center'>Log In</th>
                                    ...
                                    <tbody class="text-gray-700 text-sm">
                                        <?php
                                        try {
                                            $sql_students = "SELECT fname, lname, date_attended FROM userinfo WHERE section = :section AND date(date_attended) = curdate() AND fname IS NOT NULL AND TRIM(fname) <> '' AND lname IS NOT NULL AND TRIM(lname) <> '' ORDER BY lname ASC";
                                        ...
                                        ?>
                                            ...
                                            ?>
                                            <tr>
                                                <td colspan="5" class="border px-4 py-3 text-center text-red-500 font-semibold">Error data for this PLATFORM.</td>
                                            </tr>
                                        <?php
                                        }
                                        ?>
        ...
        if (!$message && !$any_section_displayed) {
            if ($initial_sections_found) {
                ...
                $no_data_message = "No Users today.";
            }
                ?>
